...
const buildSlideshow = (foods) => {
    ...
    */
    ...
    prevButton.addEventListener('click', () => {
        ...
        if (rightI >= foods.length) {
            rightI = 0;
        }
        if (leftI >= foods.length) {
            leftI = 0;
        }

        // Remove the first element 
        slideshowContainer.removeChild(slideshowContainer.children[0]);
        slideshowContainer.append(buildSlide(foods[rightI]));
    });
    ...
    nextButton.addEventListener('click', () => {
        ...
        rightI -= 1;
        if (leftI < 0) {
            leftI = foods.length - 1;
        }
        if (rightI < 0) {
            rightI = foods.length - 1;
        }
        slideshowContainer.removeChild(slideshowContainer.querySelectorAll('article')[slideCount - 1]);
        slideshowContainer.prepend(buildSlide(foods[leftI]));
    });
};

const buildSlide = (food) => {
    const foodContainer = document.createElement('article');
    if (food.fields.poster) {
        // console.log(movie.fields.poster[0].url);
        const posterImg = document.createElement('img');
        posterImg.src = food.fields.poster[0].url;
        posterImg.classList.add('poster-img', 'dlkjfdl');
        posterImg.id = 'poster-img-id';
        foodContainer.append(posterImg);
    }
    if (food.fields.release_date) {
        // console.log(movie.fields.release_date);
    }

    if (food.fields.description) {
        const descriptionEl = document.createElement('p');
        descriptionEl.innerHTML = food.fields.description;
        descriptionEl.classList.add('food-description');
        foodContainer.append(descriptionEl);
    }
